...
This repository contains simple but effective solution for detecting hallucinations in generated text, leveraging the power of transformer-based language models. Hallucinations, in this context, refer to incorrect or misleading information produced by models when generating responses to given queries. I am framing the task of hallucination detection as a classification problem using a pre-trained BERT model. Fine-tuning  pre-trained BERT model on my specific dataset benefits  by leveraging the extensive language understanding BERT gained from being pre-trained on large amounts of diverse text data, which enhances its ability to accurately detect hallucinations.

## Dataset Description
The dataset is structured to support the task of detecting hallucinations in text. It consists of the following columns:
